...
$results = $dbq->query('SELECT id,timestamp,username,string FROM recentactivity ORDER BY timestamp DESC LIMIT 10');
while ($row = $results->fetchArray()) {
  echo "  <entry>\n";
  $time = round((($unixtime-htmlentities($row['timestamp']))/60),0);
  echo "<title type=\"html\">A user ".htmlentities($row['string'], ENT_XML1)." &lt;small&gt;".$time." minutes ago.&lt;/small&gt;</title>\n"; // is used by dokuwiki // ENT_XML1 so things like &ldquo; is output as numeric entity rather, that is allowed, the name isnt
